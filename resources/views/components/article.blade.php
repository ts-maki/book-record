    ...
    <div class="flex flex-col py-2 px-4 flex-1 justify-between">
        ...
        <div class="flex justify-between items-center">
            ...
            <form action="{{ route('favorite.save',  ['record_id' => $record->id]) }}" method="post">
                @csrf
                <button type="submit" class="px-2 border-yellow-300 border-solid border-2 rounded-full hover:text-white hover:bg-yellow-300 duration-300">お気に入り登録</button>
            </form>
            @else
            <form action="{{ route('favorite.destroy', ['record_id' => $record->id]) }}" method="post">
                @csrf
                @method('DELETE')
                <button type="submit" class="px-2 border-red-300 border-solid border-2 rounded-full hover:text-white hover:bg-red-300 duration-300">お気に入り削除</button>
            </form>
            @endif
            @endauth
            ...
            <div>
                <a href="{{ route('other.book.record', ['id' => $record->book->id]) }}"
                    class="px-2 border-green-200 border-solid border-2 rounded-full hover:text-white hover:bg-green-300 duration-300">この本を感想に登録する</a>
            </div>
            @endif
            @if (Auth::id() === $record->user_id)
            <div class="flex">
